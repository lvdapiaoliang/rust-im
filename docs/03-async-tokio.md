...
| 嵌入 SDK 内的运行时（阶段 11） | 单线程/独立实例 | 避免和宿主 App 抢线程 |
...
### 5.5 io_uring（Linux 专属，阶段 10 M2 里程碑主角）
...
阶段 10 用 `tokio-uring` 做对照实验，数据进 docs/16。
...
> Tokio 主线基于 epoll/IOCP 的就绪通知模型，每次 IO 至少两次系统调用（注册兴趣+收取事件）。io_uring 用内核/用户态共享的提交队列（SQ）与完成队列（CQ），支持批量提交、零拷贝收割，还提供异步文件、超时等统一接口。tokio-uring 是基于 io_uring 的独立运行时（单线程模型）。我在阶段 10 的计划是用它对网关做 10 万/100 万连接对照压测，量化每连接内存与 syscall 次数的差距——这是 M2 里程碑的核心实验。
