...
客户端至此拿到了完整的可靠性闭环，但「P99 < 10ms、10 万并发连接」
还只是口号。阶段 5 进入 `im-bench`：连接风暴发生器、消息吞吐基准、
用户态弱网模拟（延迟 + 丢包 + 乱序注入），以及每一级里程碑的
量化优化记录（火焰图 + 前后对比数据）。本阶段的 Outbox/去重/游标
就是弱网模拟器要轰炸的对象——**先有正确性，再谈性能**。
