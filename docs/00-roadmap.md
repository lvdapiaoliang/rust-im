...
| 里程碑 | 指标 | 主要手段 | 状态 |
|--------|------|----------|------|
| M1 | 单机 10 万并发连接 | 常规 Tokio + 参数调优 | ✅ 达成：99,969 连接、每连接 29.59 KiB（双端）、拆除 6.02s 清零（docs/16） |
| M2 | 单机 100 万并发连接 | `io_uring`、内核调优、每连接内存压到 KB 级 | 未开始（Linux 平台课题） |
| M3 | 单机 500 万并发连接 | `SO_REUSEPORT` 多进程、内存池、零拷贝（极限挑战） | 未开始 |
| 延迟 | 消息端到端 P99 < 10ms（同机房） | 背压、锁竞争消除、批量化 | 未测（无同机房场景） |
| 可靠 | 100ms RTT + 10% 丢包下到达率 99.999% | ACK + 指数退避重传 + 去重 | 部分达成：上行 100%（200/200）、下行单次 90%（投递不重传为已知边界，docs/16 §5.6） |
...
mdbook serve                  # 本地预览文档站（阶段 14；产物 book/ 不入库）
